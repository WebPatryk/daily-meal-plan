...
/**
 * Checks if an image_path string contains an icon reference
 */
export function isIconPath(imagePath: string | null | undefined): boolean {
  return !!imagePath && imagePath.startsWith("icon:");
}

/**
 * Extracts icon name from an icon path (format: "icon:category")
 */
...
export function getMealIcon(iconName: MealIconName, className: string = "w-16 h-16 text-primary"): JSX.Element {
  switch (iconName) {
    ...
    case "salad":
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 12a8 8 0 11-16 0 8 8 0 0116 0zm-8-4v8m-4-4h8" />
        </svg>
      );
    case "meat":
      return (
